...
5. **Cupom + cashback (carteira)** — `domain/cupom.py` + `domain/cashback.py` (melhor cupom válido; melhor cashback elegível por `CASHBACK_ELEGIVEL` no `.env`), tabelas SQLite, comandos CLI (`cupom`/`cashback`), endpoints `/api/carteira` (+ POST/DELETE cupom/cashback). Preço final = item + frete − cupom − cashback (§16), **reaplicado a cada leitura** (`_ofertas_guardadas`) — adicionar/remover cupom e recarregar já muda o preço, sem re-buscar. UI: **escadinha** (base − cupom − cashback = final) por oferta + tela **Carteira**. `OfertaView`/`OfertaRankeada` carregam a decomposição. **Cashback é manual** (elegibilidade pessoal, atrás de login — não dá pra descobrir automático).
6. **Descoberta automática de CUPONS** (o Rodrigo não digita cupom) — porta `application/buscadores.py` (`BuscadorDeCupons` + `CupomDescoberto`) + adaptador `adapters/cupons/serper_llm.py`: na busca, **por loja que apareceu no ranking**, roda Serper `"cupom <loja> desconto"` → LLM extrai códigos + categorias dos snippets (Cuponomia/Pelando/Méliuz). **Validação por SINAIS** (checkout é proibido, então não é garantia): status `provavel_valido`/`nao_confirmado`/`expirado` por validade + frescor + corroboração entre fontes; guarda anti-clickbait (% ≥50 não auto-aplica). **Cache por TTL (24h)** por loja (não redescobre a cada busca; colunas `origem/status/confianca/evidencias/descoberto_em/categorias` na tabela `cupom`). Aplica **automático** o melhor `provavel_valido`, **marcado `cupom_confirmado=False`** (a UI avisa "não confirmado"), e **só na categoria certa** (cupom de "celular" não desconta geladeira — `Cupom.aplica_na_categoria`). UI: escadinha nomeia o cupom + status; card da oferta lista os cupons da loja **copiáveis** ao lado de "Abrir na loja"; carteira mostra descobertos por loja com status/evidências/categorias + 🗑. Sem `SERPER_API_KEY`/`NVIDIA_API_KEY` → só carteira manual.

**Aberto / próximo:** Serper free-tier varia muito por chamada (1–13 lojas) — o `buscar` acumula as validadas no banco; consistência real pede upgrade do Serper ou 2ª fonte. **Descoberta de cashback** (taxa pública do Méliuz por loja, no mesmo molde do cupom) fica pra depois. Gate de capacidade (ex.: geladeira 375L vs 385L passou pra revisão em vez de descartar), unificar "Mercado Livre" × "mercadolivre.com.br", cache de query + contador de créditos Serper, gráfico de evolução 30 dias, e `primeira_compra` (cupom só-1ª-compra nunca aplica hoje) ficam pendentes.
